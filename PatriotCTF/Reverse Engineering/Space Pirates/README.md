# Space Pirates
<img width="533" height="493" alt="image" src="https://github.com/user-attachments/assets/081730a5-75ea-4ab3-9113-36749d67760d" />


### **Mô tả:**
Decode thông điệp của hải tặc không gian bằng cách đảo ngược 4 phép biến đổi tuần tự (XOR key, swap byte, cộng hằng số, XOR index). Mục tiêu là tìm input tạo ra TARGET[] — chính là flag.
### Phân tích:
...
void print_flag(char *input) {
    ...
    printf("  *    .   *    TREASURE!   .   *\n");
    printf("\n");
    printf("ðŸ´â€â˜ ï¸  DECRYPTION SUCCESSFUL! ðŸ´â€â˜ ï¸\n");
    printf("\n");
    printf("Flag: %s\n",input);
    printf("\n");
}
...
Chương trình kiểm tra độ dài 30 byte (FLAG_LEN)
Sau đó, mỗi byte sẽ đi qua 4 phép biến đổi
  1) XOR với key xoay vòng XOR_KEY[i % 5]
  2) Swap từng cặp byte
  3) Cộng MAGIC_ADD (0x2A)
  4) XOR với vị trí i

Sau khi biến đổi xong, kết quả phải bằng đúng mảng TARGET.
...
```py
#!/usr/bin/env python3
# Reverse Space Pirate CTF Encryption
...
def reverse():
    ...
    # STEP 4 (reverse): XOR với vị trí
    for i in range(FLAG_LEN):
        buf[i] ^= i

    # STEP 3 (reverse): subtract MAGIC_ADD
    for i in range(FLAG_LEN):
        buf[i] = (buf[i] - MAGIC_ADD) & 0xFF

    # STEP 2 (reverse): swap pairs lại
    for i in range(0, FLAG_LEN, 2):
        buf[i], buf[i+1] = buf[i+1], buf[i]

    # STEP 1 (reverse): XOR lại với rotating key
    for i in range(FLAG_LEN):
        buf[i] ^= XOR_KEY[i % 5]
